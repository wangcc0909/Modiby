...
func main() {
	startTime := time.Now()
	var url string //文件下载的地址
	url = "http://localhost:9090/download/goland-2020.2.3.dmg"
...
//head 获取要下载文件的基本信息(header) 使用HTTP Method Head
...
func (d FileDownloader) downloadPart(c filePart) error {
	...
	if resp.StatusCode > 299 {
		return errors.New(fmt.Sprintf("服务器错误,状态码: %v", resp.StatusCode))
	}
...
func (d FileDownloader) mergeFileParts() error {
	...
	if hex.EncodeToString(hash.Sum(nil)) != "bbd73cadabb7bb16b04f0a539c5a9093126c38b427615d48e8dab25ae1a32f45" {
